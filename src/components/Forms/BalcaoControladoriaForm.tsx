...
const BalcaoControladoriaForm = () => {
  ...
  // Pré-selecionar o usuário logado quando os usuários forem carregados
  useEffect(() => {
    if (user?.id && usuarios.length > 0 && !formData.nomeSolicitante) {
      const usuarioLogado = usuarios.find(u => u.id === user.id);
      if (usuarioLogado) {
